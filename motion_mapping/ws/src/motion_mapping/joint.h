...
class Joint {
 public:
   ...
   */
  void add_frame_motion_data(const std::vector <double>& data) {
      //delete_frame_motion_data();
      channel_data_.push_back(data);
  }

  void delete_frame_motion_data(){
      if (!channel_data_.empty()){
      channel_data_.begin()->clear();
      channel_data_.begin()->shrink_to_fit();
      std::cout << channel_data_.empty() << std::endl;
      channel_data_.clear();
      }
  }

  void set_chananel_data(unsigned channel_num, double value){
      channel_data_.at(0).at(channel_num) = value;
  }
   ...
   */
  const std::vector <double>& channel_data(unsigned frame) const {
   ...
   */
  double channel_data(unsigned frame, unsigned channel_num) const {
   ...
   */
  void set_channel_data(const std::vector <std::vector <double>>& arg) {
    channel_data_ = arg;
  }
 ...
 private:
   ...
   */
  std::vector <std::vector <double> > channel_data_;
