...
class Bvh_parser {
 public:
   ...
   */
    ...
    int parse(const bf::path& path, Bvh* bvh2, const char* MotionData, int size) {
      ...
      if (file.is_open()) {
        ...
        while (file.good()) {
          ...
          if (token == kHierarchy) {
            ...
            //std::cout << "Press Enter to continue...kHierarchy";
            //std::cin.ignore();
            const char* motionData = MotionData;
            int bvhDataSize = 2100;
            ret = parse_motion_package(motionData, bvhDataSize);
            if (ret)
              return ret;
          } else {
    ...
    int parse_motion_package(const char* MotionData, int size) {
     ...
     // LOG(INFO) << "Num of frames : " << frames_num;
      ...
      double number;
      if (first_flag){
          //std::cout << "Press Enter to continue...first_flag";
         
          first_flag = false;
      for (auto joint : bvh_->joints()) {

          //std::cout << "Press Enter to continue...no_ways";
          //std::cin.ignore();
          ...
          std::cout << "Press Enter to continue...motion";
          //std::cin.ignore();
          for (auto& joint : bvh_->joints()) {

              std::cout << "Joint: " << joint->name() << ", Channel Size: " << joint->num_channels() << std::endl;
    ...
    if (token == kFrame) {
     ...
     // LOG(ERROR) << "Bad structure of .bvh file. Expected " << kFrame
      //           << ", but found \"" << token << "\"";
        ...
        std::cerr << "Bad structure of .bvh file. Expected " << kFrame
